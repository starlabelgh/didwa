...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content container-fluid">
      <div class="row">
        <!-- get this month income query -->
        <?php
            $select = $pdo->prepare("SELECT sum(total) as total FROM tbl_invoice WHERE MONTH(order_date) = MONTH(CURDATE())");
            $select->execute();
        ...
        <div class="col-md-3 col-sm-6 col-xs-12">
          <div class="info-box">
            ...
            <div class="info-box-content">
              ...
              <span class="info-box-number"><small>GH₵ <?php echo number_format($total,0); ?></small></span>
            </div>
            <!-- /.info-box-content -->
          </div>
          <!-- /.info-box -->
      ...
      <div class="row">
        ...
        <!--total expense this month begin-->
        <?php
            $select = $pdo->prepare("SELECT sum(exp_amount) as total FROM tbl_expense WHERE MONTH(exp_date) = MONTH(CURDATE())");
            $select->execute();
            $row=$select->fetch(PDO::FETCH_OBJ);
            $total = $row->total;
        ?>
         <!-- get this months expense -->
        <div class="col-md-3 col-sm-6 col-xs-12">
          <div class="info-box">
            <span class="info-box-icon bg-aqua"><i class="fa fa-money"></i></span>
        ...
        <!-- get today transactions -->
        <?php
        $select = $pdo->prepare("SELECT sum(delivery) as total FROM tbl_invoice WHERE MONTH(order_date) = MONTH(CURDATE())");
            $select->execute();
            $row=$select->fetch(PDO::FETCH_OBJ);
            $total = $row->total;
        ?>
        ...
        <div class="col-md-3 col-sm-6 col-xs-12">
          <div class="info-box">
            ...
            <div class="info-box-content">
              <span class="info-box-text">Courier This Month</span>
              <span class="info-box-number"><small>GH₵ <?php echo number_format($total,0); ?></small></span>
            </div>
            <!-- /.info-box-content -->
          </div>
                  ...
                  <table class="table table-striped" id="myBestProduct">
                      ...
                      <tbody>
                          ...
                          ?>
                              ...
                              <td>GH₵ <?php echo number_format($row->price);?></td>
                              <td>GH₵ <?php echo number_format($row->total); ?></td>
                              </tr>
